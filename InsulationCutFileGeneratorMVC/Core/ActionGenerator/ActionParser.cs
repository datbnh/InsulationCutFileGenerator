...
namespace InsulationCutFileGeneratorMVC.Core.ActionGenerator
{
    ...
    {
        public static void GetParset(Action action)
        {
            switch (action)
            {
        ...
        private static List<KeyValuePair<Action, object[]>> DecomposeRipCutBackward(KeyValuePair<Action, object[]> action)
        {
            ...
            output.Add(new KeyValuePair<Action, object[]>(Action.MoveTo, new object[] { DataEntry.DUCT_FULL_LENGTH, x }));
            output.Add(new KeyValuePair<Action, object[]>(Action.KnifeDown, null));
            output.Add(new KeyValuePair<Action, object[]>(Action.MoveTo, new object[] { 0, x }));
        ...
        private static List<KeyValuePair<Action, object[]>> DecomposeHorizontalCutBackward(KeyValuePair<Action, object[]> action)
        {
            ...
            if (action.Value == null || action.Value.Length < 3)
                throw new ArgumentException("Invalid HorizontalCutBackward action argument: this action must take at least three arguments.");
            var x = (long)(int)(action.Value[0]);
            var y = (long)(int)(action.Value[1]);
            var length = (long)(int)(action.Value[2]);
